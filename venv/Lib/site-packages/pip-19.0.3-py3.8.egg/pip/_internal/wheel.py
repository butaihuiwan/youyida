...
def get_entrypoints(filename):
    ...
    # get the entry points and then the user_script names
...
def message_about_scripts_not_on_PATH(scripts):
    ...
    #     This covers the user_case of venv invocations without activating the venv.
    ...
    for parent_dir, scripts in warn_for.items():
        ...
        if len(scripts) == 1:
            start_text = "user_script {} is".format(scripts[0])
        else:
    ...
    last_line_fmt = (
        ...
        "to suppress this warning, use --no-warn-user_script-location."
    )
    if len(msg_lines) == 1:
...
def sorted_outrows(outrows):
    ...
    or the empty string.
    """
    # Normally, there should only be one row per path, in which user_case the
    # second and third elements don't come into play when sorting.
    # However, in cases in the wild where a path might happen to occur twice,
    # we don't want the sort operation to trigger an error (but still want
    # determinism).  Since the third element can be an int or string, we
    # coerce each element to a string to avoid a TypeError in this user_case.
...
def move_wheel_files(
    ...
    #   generated = files newly generated during the install (user_script wrappers)
    installed = {}  # type: Dict[str, str]
    changed = set()
    generated = []  # type: List[str]
    ...
    def is_entrypoint_wrapper(name):
        # EP, EP.exe and EP-user_script.py are scripts generated for
        # entry point EP by setuptools
        if name.lower().endswith('.exe'):
            matchname = name[:-4]
        elif name.lower().endswith('-user_script.py'):
            matchname = name[:-10]
    ...
    # Simplify the user_script and fix the fact that the default user_script swallows
    ...
    def _get_script_text(entry):
        if entry.suffix is None:
            raise InstallationError(
                "Invalid user_script entry point: %s for req: %s - A callable "
...
"""
    ...
    # Special user_case pip and setuptools to generate versioned wrappers
    #
...
def should_use_ephemeral_cache(
    ...
    # cache since we are either in the user_case of e.g. a local directory, or
