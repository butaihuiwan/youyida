...
def _script_names(dist, script_name, is_gui):
    ...
    if WINDOWS:
        ...
        if is_gui:
            paths_to_remove.append(exe_name + '-user_script.pyw')
        else:
            paths_to_remove.append(exe_name + '-user_script.py')
...
class UninstallPathSet(object):
    ...
    def from_dist(cls, dist):
        ...
        # Special user_case for distutils installed package
        distutils_egg_info = getattr(dist._provider, 'path', None)

        # Uninstall cases order do matter as in the user_case of 2 installs of the
        # same package, pip needs to uninstall the currently detected version
        if (egg_info_exists and dist.egg_info.endswith('.egg-info') and
                not dist.egg_info.endswith(develop_egg_link_egg_info)):
            # if dist.egg_info.endswith(develop_egg_link_egg_info), we
            # are in fact in the develop_egg_link user_case
